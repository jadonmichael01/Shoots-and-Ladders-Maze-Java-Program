...
public class Maze {
	...
	private void solveHelper(Cell source, Cell target) {
		...
		if(source.deadend){
			source.shortest = Integer.MAX_VALUE / 2;
			return;
		}
		...
		if(source.row < rows - 1 && !map[source.row + 1][source.col].visited && !source.noDown)
			solveHelper(map[source.row + 1][source.col], target);
		if(source.col < columns - 1 && !map[source.row][source.col + 1].visited && !source.noRight)
			solveHelper(map[source.row][source.col + 1], target);
		...
		if(source.row == rows - 1 && !source.noRight){
			...
			source.shortest = 1000000;
		else {
			Cell rightNeighbor = map[source.row][source.col + 1], bottomNeighbor = map[source.row + 1][source.col];
			int bigVal = 1000000;
			int rightPath = source.noRight || rightNeighbor.deadend ? bigVal : rightNeighbor.shortest;
			int downPath = source.noDown || bottomNeighbor.deadend ? bigVal : bottomNeighbor.shortest;
